...
首次启动会弹出设置向导，按下面的顺序填写（无桌面时命令行也是同一顺序，每行一个答案；直接回车表示接受方括号里的默认值）：
...
| 4 | 端口 | 默认 5000；若该端口被占用，向导会自动建议其他端口。 |
| 5 | 管理员密码 | 至少 8 个字符，自行设置后不会打印或丢失。 |
| 6 | 再输入一次密码 | 两次必须一致。 |

学生自行注册时，用户名必须是 **3–32** 个字符，仅限小写字母、数字、`.`、`-` 或 `_`。

向导结束后才会初始化数据库并导入内置题库。导入会改写仓库里的 `data/problems/` 测试点文件，这是正常现象，不必提交。之后每次启动直接进入服务状态并打印局域网地址。
...
相同的信息以命令行问答形式呈现。所有配置保存在 `.env` 文件中，后续可直接编辑修改。
...
也可以在部署助手窗口中通过 **Initialize / repair** 重新执行初始化。这条路径不会弹出设置向导，会为 `admin` 账户生成一次性临时密码并打印在部署日志里，请保存并在首次登录时修改。双击 **启动 EasyOJ.bat** 的首次安装则使用向导里你自己设定的密码，不会再打印临时密码。
