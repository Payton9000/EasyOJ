...
首次运行会自动完成所有准备工作：创建项目内的 `.venv` 虚拟环境、安装本地的 MinGW、JDK 和 Python 提交运行时，生成 `SECRET_KEY`，初始化数据库和内置题库。全程不需要修改系统 `PATH`，不需要预装全局编译器。首次安装会先从国内镜像下载 MinGW、JDK 和 Python 运行时（SHA-256 校验失败再回退官方源），合计约 400MB；网络很慢时可能超过一小时。若第一次被中断，再次双击本文件会继续补齐工具链。之后每次启动直接进入服务状态并打印局域网地址。
