...
也可以双击根目录的 **启动 EasyOJ.bat**：首次运行会走同样的准备流程并弹出设置向导。工具链下载默认先走国内镜像（ghfast / 华为云 Python），校验失败再回退 GitHub 与 python.org；启动器超时为 3 小时。中途失败后再次启动会继续补齐缺失的编译器，不会因为已经有 `.venv` 而跳过。
