...
也可以双击根目录的 **启动 EasyOJ.bat**：首次运行会走同样的准备流程并弹出设置向导。工具链会先对官方源和多个国内镜像测速，再从最快的源下载（校验失败则按测速顺序回退；传输中断会断点续传，假 ZIP 会被丢弃）。若 5000 已被其他程序占用，向导会建议别的端口，启动器不会把别人的实例当成自己已经启动。启动器超时为 3 小时。中途失败后再次启动会继续补齐缺失的编译器，不会因为已经有 `.venv` 而跳过。
