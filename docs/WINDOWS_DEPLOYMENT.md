...
and five concurrent users. The load command performs bounded GET requests only. It refuses values above
30 seconds, 12 users, 2 workers, or 100 requests. Judge stress tests require
an explicit operator decision and should never be run during a class.
