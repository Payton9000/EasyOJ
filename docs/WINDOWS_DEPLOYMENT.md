...
教室里的默认做法是双击根目录的 **启动 EasyOJ.bat**：首次运行会先准备工具链，再打开本机浏览器中的配置页（管理员用户名、密码、邮箱、站点名称、端口；只监听 127.0.0.1），提交后才初始化数据库。工具链会先对官方源和多个国内镜像测速，再从最快的源下载（校验失败则按测速顺序回退；传输中断会断点续传，假 ZIP 会被丢弃）。若 5000 已被其他程序占用，配置页会提示改用别的端口，启动器不会把别人的实例当成自己已经启动。启动器超时为 3 小时。中途失败后再次启动会继续补齐缺失的编译器，不会因为已经有 `.venv` 而跳过。

如果还没有项目内的 `.venv`，也可以只借用本机 Python 一次来创建它；之后应用依赖、学生提交用的嵌入式 Python、MinGW 和 JDK 都放在项目目录里。`.venv` 只运行网站，不用来执行不受信任的学生代码。

需要开机自启、托盘和修工具链时，再打开部署助手：
...
选择 **Repair toolchain** 只会补齐编译器和 `.venv`，**不会**创建管理员。首次开户请用 **启动 EasyOJ.bat** 的浏览器配置页。可以重复执行修复；不会修改系统 PATH。仅修复工具链时，仍可使用旧的 `setup_toolchain.ps1`。
...
双击 **启动 EasyOJ.bat** 的首次安装使用配置页里设定的管理员密码。部署助手不再生成一次性临时管理员密码。需要受控的初始密码时可在网页里自行填写。之后用管理后台重置流程发放临时密码。重置页标记为 `no-store`，并强制对方下次改密。
